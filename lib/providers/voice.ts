...
export type VoiceIdentity = VoiceCharacteristics & { primary: { model: string; voice: string }; fallback: { model: string; voice: string } };
export type SpeechToTextResult = { text: string; providerId: string; modelId: string; requestId: string | null; usage: Record<string, unknown>; fallbackUsed?: boolean; fallbackReason?: string };
export type TextToSpeechResult = { audio: Uint8Array; contentType: string; providerId: string; modelId: string; requestId: string | null; usage: Record<string, unknown>; fallbackUsed?: boolean; fallbackReason?: string };

type LiveVoiceModel = { id: string; inputPrice: number; outputPrice: number; contextLength: number; architecture?: { input_modalities?: string[]; output_modalities?: string[] }; supportedParameters?: string[] };
type RankedVoiceModel = { model: LiveVoiceModel; score: number };
export interface SpeechToTextProvider { readonly id: string; transcribe(input: { audioBase64: string; format: string; language?: string; modelId?: string }): Promise<SpeechToTextResult> }
export interface TextToSpeechProvider { readonly id: string; synthesize(input: { text: string; voice: string; modelId: string; locale: string; instructions?: string }): Promise<TextToSpeechResult> }
...
function apiKey() { const key = process.env.OPENROUTER_API_KEY; if (!key) throw new Error('OPENROUTER_API_KEY_MISSING'); return key; }
function headers() { return { Authorization: `Bearer ${apiKey()}`, 'Content-Type': 'application/json', 'HTTP-Referer': process.env.APP_URL || 'https://horus-os.com', 'X-Title': 'Hórus Cognitive OS' }; }
function timeoutSignal() { return AbortSignal.timeout(REQUEST_TIMEOUT_MS); }
function numericPrice(value: unknown) { const parsed = Number(value); return Number.isFinite(parsed) && parsed >= 0 ? parsed : Number.POSITIVE_INFINITY; }
function normalizeModality(value: unknown) { return typeof value === 'string' ? value.toLowerCase().trim() : ''; }
function modelSupports(model: LiveVoiceModel, input: InputModality, output: VoiceOutput) {
  ...
  return (!inputs.length || inputAliases.some((candidate) => inputs.includes(candidate))) && (!outputs.length || outputAliases.some((candidate) => outputs.includes(candidate)));
}
...
function localeScore(characteristics: VoiceCharacteristics) { return characteristics.locale.toLowerCase().startsWith('pt') ? 1 : 0.55; }
function rankVoiceModels(models: LiveVoiceModel[], characteristics: VoiceCharacteristics, requiredInputModality: InputModality, output: VoiceOutput): RankedVoiceModel[] {
  ...
  return models.filter((model) => modelSupports(model, requiredInputModality, output)).filter(economicEligibility).map((model) => {
    ...
    return { model, score: 0.30 + costScore * 0.30 + contextScore * 0.10 + locale * 0.15 + reliabilityScore * 0.15 };
...
async function discoverVoiceModels(outputModality: VoiceOutput): Promise<LiveVoiceModel[]> {
  const response = await fetch(`${OPENROUTER_URL}/models?output_modalities=${outputModality}`, { headers: { Authorization: `Bearer ${apiKey()}` }, cache: 'no-store', signal: timeoutSignal() });
  ...
  return payload.data.filter((model: any) => typeof model?.id === 'string').map((model: any) => ({ id: model.id, inputPrice: numericPrice(model?.pricing?.prompt), outputPrice: numericPrice(model?.pricing?.completion), contextLength: Number(model?.context_length ?? 0), architecture: model.architecture, supportedParameters: Array.isArray(model?.supported_parameters) ? model.supported_parameters : undefined }));
}
function voiceForModel(modelId: string, preferred: unknown): string { if (typeof preferred === 'string' && preferred.trim()) return preferred.trim(); const id = modelId.toLowerCase(); if (id.includes('grok')) return 'Eve'; if (id.includes('gemini')) return 'Kore'; if (id.includes('kokoro')) return 'af_heart'; return 'nova'; }
function normalizeSttLanguage(language?: string): string | undefined { if (!language) return undefined; const normalized = language.trim().toLowerCase().replace('_', '-'); const [iso639] = normalized.split('-'); return iso639 && /^[a-z]{2}$/.test(iso639) ? iso639 : undefined; }
function retryableVoiceStatus(status: number) { return status === 400 || status === 402 || status === 408 || status === 409 || status === 429 || status >= 500; }
...
async function parseSttResponse(response: Response, selected: string, fallbackUsed = false, fallbackReason?: string): Promise<SpeechToTextResult> {
  ...
  if (!text) throw new Error(`STT_EMPTY_RESULT:${selected}`);
...
export async function resolveVoiceIdentity(characteristics: VoiceCharacteristics, preferredVoice?: unknown): Promise<VoiceIdentity> {
  const ranked = rankVoiceModels(await discoverVoiceModels('speech'), characteristics, 'text', 'speech');
...
export async function resolveSpeechToTextModel(characteristics: VoiceCharacteristics): Promise<{ primary: string; fallback: string }> {
  const ranked = rankVoiceModels(await discoverVoiceModels('transcription'), characteristics, 'audio', 'transcription');
...
export class OpenRouterSpeechProvider implements SpeechToTextProvider {
  ...
  async transcribe(input: { audioBase64: string; format: string; language?: string; modelId?: string }): Promise<SpeechToTextResult> {
    ...
    const ranked = rankVoiceModels(await discoverVoiceModels('transcription'), { locale: input.language || 'pt-BR', gender: 'neutral' }, 'audio', 'transcription');
    const candidates = [...new Set(input.modelId ? [input.modelId, ...ranked.map((entry) => entry.model.id)] : ranked.map((entry) => entry.model.id))];
    if (!candidates.length) throw new Error('STT_CATALOG_NO_COMPATIBLE_MODEL');
    ...
    for (let index = 0; index < candidates.length; index += 1) {
      ...
      try {
        ...
        const empty = message.startsWith('STT_EMPTY_RESULT:');
        if ((!retryableVoiceStatus(status) && !empty) || index === candidates.length - 1) throw error;
      }
    }
...
export class OpenRouterSpeechSynthesisProvider implements TextToSpeechProvider {
  ...
  async synthesize(input: { text: string; voice: string; modelId: string; locale: string; instructions?: string }): Promise<TextToSpeechResult> {
    ...
    const ranked = rankVoiceModels(await discoverVoiceModels('speech'), { locale: input.locale, gender: 'neutral' }, 'text', 'speech');
    const candidates = [...new Set([input.modelId, ...ranked.map((entry) => entry.model.id)])];
    let lastError: unknown;
    for (let index = 0; index < candidates.length; index += 1) {
      const selected = candidates[index];
      const voice = index === 0 ? input.voice : voiceForModel(selected, input.voice);
      try {
        const response = await fetch(`${OPENROUTER_URL}/audio/speech`, { method: 'POST', headers: headers(), body: JSON.stringify({ model: selected, input: input.text, voice, response_format: 'mp3', instructions: input.instructions }), cache: 'no-store', signal: timeoutSignal() });
        if (!response.ok) {
          const payload = await response.json().catch(() => ({}));
          const error = new Error(`TTS_PROVIDER_FAILED:${response.status}:${payload?.error?.message || payload?.error?.code || 'UNKNOWN'}:${selected}`);
          lastError = error;
          if (!retryableVoiceStatus(response.status) || index === candidates.length - 1) throw error;
          continue;
        }
        const audio = new Uint8Array(await response.arrayBuffer());
        if (!audio.length) { lastError = new Error(`TTS_EMPTY_RESULT:${selected}`); if (index === candidates.length - 1) throw lastError; continue; }
        ...
        const status = statusMatch ? Number(statusMatch[1]) : 0;
        const empty = message.startsWith('TTS_EMPTY_RESULT:');
        if ((!retryableVoiceStatus(status) && !empty) || index === candidates.length - 1) throw error;
      }
    }
...
export function getSpeechToTextProvider(providerId = 'openrouter'): SpeechToTextProvider { if (providerId === 'openrouter') return new OpenRouterSpeechProvider(); throw new Error(`STT_PROVIDER_UNSUPPORTED:${providerId}`); }
export function getTextToSpeechProvider(providerId = 'openrouter'): TextToSpeechProvider { if (providerId === 'openrouter') return new OpenRouterSpeechSynthesisProvider(); throw new Error(`TTS_PROVIDER_UNSUPPORTED:${providerId}`); }
