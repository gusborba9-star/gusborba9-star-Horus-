...
const HorusState = Annotation.Root({
  ...
  requiresHuman: Annotation<boolean>, action: Annotation<string>, error: Annotation<string | undefined>, economicAuthorized: Annotation<boolean>,
...
function assessConfidence(state: HorusCoreState): Partial<HorusCoreState> { if (state.error) return { confidence: 0, requiresHuman: true, action: 'invalid_request' }; let confidence = 0.55; if (state.payload.intent) confidence += 0.15; if (state.payload.operation) confidence += 0.10; if (state.memoryContext.length > 0) confidence += 0.10; if (state.source) confidence += 0.05; if (state.payload.request_id) confidence += 0.05; confidence = Math.min(1, confidence); const approved = state.humanApproval?.approved === true && Boolean(state.humanApproval.reviewId); return { confidence: approved ? Math.max(confidence, 0.7) : confidence, requiresHuman: approved ? false : confidence < 0.7, action: approved ? 'route_to_service' : confidence < 0.7 ? 'human_review' : 'route_to_service' }; }
...
export async function runHorusCore(input: HorusCoreInput): Promise<HorusCoreState> { return graph.invoke({ eventType: input.event_type ?? '', payload: input.payload ?? {}, source: input.source ?? '', humanApproval: input.humanApproval, confidence: 0, memoryContext: [], requiresHuman: false, action: 'pending', error: undefined, economicAuthorized: false, executionAttemptId: undefined, executionBudgetId: undefined, pricingSnapshotId: undefined, endpointId: undefined, routedProviderId: undefined, routedModelId: undefined, executionText: undefined, actualCostBrl: undefined, usage: undefined }); }
