...
export type HorusCoreInput = { event_type?: string; payload?: Record<string, unknown>; source?: string };
...
const HorusState = Annotation.Root({
  ...
  memoryContext: Annotation<unknown[]>({ reducer: (_current, update) => update, default: () => [] }), requiresHuman: Annotation<boolean>, action: Annotation<string>,
  error: Annotation<string | undefined>, economicAuthorized: Annotation<boolean>, executionAttemptId: Annotation<string | undefined>, executionBudgetId: Annotation<string | undefined>,
  pricingSnapshotId: Annotation<string | undefined>, endpointId: Annotation<string | undefined>, routedProviderId: Annotation<string | undefined>, routedModelId: Annotation<string | undefined>,
  executionText: Annotation<string | undefined>, actualCostBrl: Annotation<number | undefined>, usage: Annotation<Record<string, unknown> | undefined>,
});
...
function validateInput(state: HorusCoreState): Partial<HorusCoreState> { if (!state.eventType) return { error: 'event_type é obrigatório', confidence: 0, requiresHuman: true, action: 'invalid_request' }; return { error: undefined }; }
...
function assessConfidence(state: HorusCoreState): Partial<HorusCoreState> { if (state.error) return { confidence: 0, requiresHuman: true, action: 'invalid_request' }; let confidence = 0.55; if (state.payload.intent) confidence += 0.15; if (state.payload.operation) confidence += 0.10; if (state.memoryContext.length > 0) confidence += 0.10; if (state.source) confidence += 0.05; if (state.payload.request_id) confidence += 0.05; confidence = Math.min(1, confidence); return { confidence, requiresHuman: confidence < 0.7, action: confidence < 0.7 ? 'human_review' : 'route_to_service' }; }
...
async function authorizeEconomicExecution(state: HorusCoreState): Promise<Partial<HorusCoreState>> {
  if (state.error || state.requiresHuman || state.action !== 'route_to_service') return { economicAuthorized: false };
  ...
  if (!budgetId || !input) return { economicAuthorized: false, action: 'economic_authorization_required', error: 'economic_authorization_requires_budget_and_input' };
  if (capability !== 'TEXT_GENERATION') return { economicAuthorized: false, action: 'provider_execution_not_supported', error: `provider_execution_not_supported:${capability}` };
  if (!Number.isSafeInteger(maxOutputTokens) || maxOutputTokens <= 0) return { economicAuthorized: false, action: 'invalid_execution_budget', error: 'INVALID_MAX_OUTPUT_TOKENS' };
  ...
  for (const candidate of candidates) {
    if (!isPricingFresh(candidate.model.priceVerifiedAt, candidate.model.expirationDate)) continue;
    pricingCandidates.push({ model: candidate.model, endpointPricing: await getEndpointPricing(candidate.provider.id, candidate.model.id, endpointId) });
  }
  if (pricingCandidates.length === 0) return { economicAuthorized: false, action: 'economic_pricing_unavailable', error: 'PRICING_STALE_OR_UNAVAILABLE' };
...
export async function runHorusCore(input: HorusCoreInput): Promise<HorusCoreState> { return graph.invoke({ eventType: input.event_type ?? '', payload: input.payload ?? {}, source: input.source ?? '', confidence: 0, memoryContext: [], requiresHuman: false, action: 'pending', error: undefined, economicAuthorized: false, executionAttemptId: undefined, executionBudgetId: undefined, pricingSnapshotId: undefined, endpointId: undefined, routedProviderId: undefined, routedModelId: undefined, executionText: undefined, actualCostBrl: undefined, usage: undefined }); }
