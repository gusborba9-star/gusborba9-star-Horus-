...
import { isPricingFresh } from './economicSafety';
...
const HorusState = Annotation.Root({
  ...
  memoryContext: Annotation<unknown[]>({ reducer: (_current, update) => update, default: () => [] }),
  ...
  pricingSnapshotId: Annotation<string | undefined>,
  endpointId: Annotation<string | undefined>,
...
function validateInput(state: HorusCoreState): Partial<HorusCoreState> {
  if (!state.eventType) return { error: 'event_type é obrigatório', confidence: 0, requiresHuman: true, action: 'invalid_request' };
...
async function retrieveMemory(state: HorusCoreState): Promise<Partial<HorusCoreState>> {
  ...
  if (!Array.isArray(embedding) || !embedding.every((value) => typeof value === 'number')) return { memoryContext: [] };
...
function assessConfidence(state: HorusCoreState): Partial<HorusCoreState> {
  if (state.error) return { confidence: 0, requiresHuman: true, action: 'invalid_request' };
  let confidence = 0.55;
  ...
  return { confidence, requiresHuman: confidence < 0.7, action: confidence < 0.7 ? 'human_review' : 'route_to_service' };
}
...
async function authorizeEconomicExecution(state: HorusCoreState): Promise<Partial<HorusCoreState>> {
  ...
  for (const candidate of pricingCandidates) {
    ...
    const result = await authorizeHorusExecution({
      ...
      endpointId: candidate.endpointPricing?.endpointId ?? endpointId,
    });
    if (result.authorized) return {
      economicAuthorized: true,
      executionBudgetId: result.budgetId,
      executionAttemptId: result.attemptId,
      pricingSnapshotId: result.pricingSnapshotId ?? snapshot.id,
      endpointId: candidate.endpointPricing?.endpointId ?? endpointId,
      routedProviderId: candidate.model.providerId,
...
export async function runHorusCore(input: HorusCoreInput): Promise<HorusCoreState> {
  return graph.invoke({
    eventType: input.event_type ?? '', payload: input.payload ?? {}, source: input.source ?? '', confidence: 0, memoryContext: [], requiresHuman: false,
    action: 'pending', error: undefined, economicAuthorized: false, executionAttemptId: undefined, executionBudgetId: undefined,
    pricingSnapshotId: undefined, endpointId: undefined, routedProviderId: undefined, routedModelId: undefined, executionText: undefined,
    actualCostBrl: undefined, usage: undefined,
  });
}
