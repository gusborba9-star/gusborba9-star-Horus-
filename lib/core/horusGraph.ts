...
async function executeProvider(state: HorusCoreState): Promise<Partial<HorusCoreState>> { if (!state.economicAuthorized || !state.executionAttemptId || !state.routedProviderId || !state.routedModelId) return {}; const input = typeof state.payload.input === 'string' ? state.payload.input : ''; const maxOutputTokens = numericPayloadValue(state.payload.max_output_tokens) ?? 1024; const maxReasoningTokens = numericPayloadValue(state.payload.max_reasoning_tokens) ?? 0; const result = await executeAuthorizedHorusText({ attemptId: state.executionAttemptId, providerId: state.routedProviderId, modelId: state.routedModelId, input, maxOutputTokens, maxReasoningTokens, temperature: numericPayloadValue(state.payload.temperature), requestId: typeof state.payload.request_id === 'string' ? state.payload.request_id : crypto.randomUUID() }); return { action: 'execution_completed', executionText: result.text, actualCostBrl: result.actualCostBrl, usage: result.usage, error: undefined }; }
