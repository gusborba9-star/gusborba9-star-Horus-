...
export async function loadPersonalContext(service: SupabaseClient, userId: string): Promise<PersonalContext> {
  ...
  const { data: subscription, error: subscriptionError } = await service.from('personal_subscriptions').select('id,tier,status,economic_profile').eq('user_id', userId).in('status', ['ACTIVE','PAST_DUE','PAUSED']).maybeSingle();
...
async function createBudget(service: SupabaseClient, userId: string, operationId: string, tier: PersonalTier) {
  ...
  const { data, error } = await service.from('execution_budgets').insert({
    ...
    revenue_allocated_brl: revenue,
    minimum_margin_rate: minimumMargin,
    maximum_provider_cost_brl: tierPolicy.maxProviderCostBrl,
    maximum_total_cost_brl: tierPolicy.maxProviderCostBrl,
    max_attempts: 1,
    max_input_tokens: 4000,
    max_output_tokens: tierPolicy.maxOutputTokens,
    max_reasoning_tokens: 0,
    max_steps: 3,
    max_tool_calls: 1,
    max_execution_seconds: 120,
    remaining_cost_brl: tierPolicy.maxProviderCostBrl,
    remaining_attempts: 1,
    remaining_input_tokens: 4000,
    remaining_output_tokens: tierPolicy.maxOutputTokens,
    remaining_reasoning_tokens: 0,
    status: 'AUTHORIZED',
    net_revenue_brl: revenue,
...
function buildPersonaSystemPrompt(context: PersonalContext, memory: Array<{ content: string }>, optimizedPrompt: string) {
  ...
  return [
    `Você é ${persona.display_name}, o Personal do usuário no Hórus Cognitive OS.`,
    `Idioma: ${persona.locale}.`,
    `Personalidade: ${JSON.stringify(persona.personality_profile)}.`,
    `Comunicação: ${JSON.stringify(persona.communication_profile)}.`,
    `Comportamento: ${JSON.stringify(persona.behavior_profile)}.`,
    'Mantenha a identidade da persona estável; nunca substitua sua voz por outra persona.',
    'Não alegue ações externas que não foram realmente executadas.',
    'Respeite permissões, autonomia, políticas e limites econômicos.',
    memoryBlock,
    `Tarefa otimizada:\n${optimizedPrompt}`,
  ].filter(Boolean).join('\n');
}
...
export async function executePersonalText(input: {
  userId: string;
  ...
  const { data: execution, error: executionError } = await service.from('personal_executions').insert({
    ...
    kind: 'CHAT',
    intent: input.intent,
    task_profile: optimized.profile,
    prompt_original: optimized.original,
  ...
  const { data: attempt, error: attemptError } = await service.rpc('authorize_horus_execution_attempt', {
    p_budget_id: budget.budgetId,
    p_attempt_number: 1,
    p_provider_id: model.providerId,
    p_model_id: model.modelId,
    p_capability: 'PERSONAL_TEXT',
    p_maximum_cost_brl: budget.maxCostBrl,
    p_input_tokens: 4000,
    p_output_tokens: budget.maxOutputTokens,
    p_reasoning_tokens: 0,
    p_endpoint_id: `${model.providerId}.text.generate`,
  ...
  try {
    ...
    const reconciled = await service.rpc('reconcile_horus_execution_attempt', {
      ...
      p_actual_provider: result.providerId,
      p_actual_model: result.modelId,
      p_latency_ms: result.latencyMs,
      p_raw_usage: result.usage.raw,
    });
