...
export class GoogleTextAdapter implements ProviderAdapter {
  ...
  async generateText(request: ProviderTextRequest): Promise<ProviderTextResponse> {
    ...
    const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(request.model)}:generateContent?key=${encodeURIComponent(apiKey)}`, {
      method: 'POST',
      signal: request.signal,
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        contents: [{ role: 'user', parts: [{ text: request.input }] }],
        ...(request.temperature === undefined && request.maxOutputTokens === undefined && request.maxReasoningTokens === undefined
          ? {}
          : { generationConfig: {
              ...(request.temperature === undefined ? {} : { temperature: request.temperature }),
              ...(request.maxOutputTokens === undefined ? {} : { maxOutputTokens: request.maxOutputTokens }),
              ...(request.maxReasoningTokens === undefined ? {} : { thinkingConfig: { thinkingBudget: request.maxReasoningTokens } }),
            } }),
      }),
    });
    ...
    return {
      ...
      usage: {
        ...
        reasoningTokens,
        cachedInputTokens: 0,
        totalTokens: data.usageMetadata?.totalTokenCount ?? inputTokens + outputTokens + reasoningTokens,
        requestCost: null,
      },
