...
function capabilityFor(model: OpenRouterModel): { id: string; category: string }[] {
  ...
  if (output.has('audio')) result.push({ id: 'MUSIC_GENERATION', category: 'generation' });
...
export async function syncOpenRouterPricing(): Promise<PricingSyncResult> {
  ...
  const { data: snapshot, error: snapshotError } = await supabase
    ...
    .upsert({
      ...
      metadata: { sync_type: 'models', version: 1 },
  ...
  for (const model of models) {
    ...
    for (const capability of capabilities) {
      ...
      const inputPrice = numeric(pricing.prompt);
      const outputPrice = numeric(pricing.completion);
      const requestPrice = numeric(pricing.request);
      const imagePrice = numeric(pricing.image);
      const reasoningPrice = numeric(pricing.internal_reasoning);
      const cachedInputPrice = numeric(pricing.input_cache_read);
      const cacheWritePrice = numeric(pricing.input_cache_write);
      ...
      const priceChanged = existing && (
        Number(existing.input_price_per_million) !== inputPrice * 1_000_000 ||
        Number(existing.output_price_per_million) !== outputPrice * 1_000_000 ||
        Number(existing.request_price) !== requestPrice ||
        Number(existing.image_price) !== imagePrice ||
        Number(existing.reasoning_price_per_million) !== reasoningPrice * 1_000_000 ||
        Number(existing.cached_input_price_per_million) !== cachedInputPrice * 1_000_000 ||
        Number(existing.cache_write_price_per_million) !== cacheWritePrice * 1_000_000
      );
      ...
      const { data: modelRow, error: modelError } = await supabase.from('models').upsert({
        ...
        input_price_per_million: inputPrice * 1_000_000,
        output_price_per_million: outputPrice * 1_000_000,
        request_price: requestPrice,
        image_price: imagePrice,
        reasoning_price_per_million: reasoningPrice * 1_000_000,
        cached_input_price_per_million: cachedInputPrice * 1_000_000,
        cache_write_price_per_million: cacheWritePrice * 1_000_000,
        currency: 'USD',
        context_window: model.context_length ?? model.top_provider?.context_length ?? null,
        ...
        metadata: { source: 'OPENROUTER_MODELS_API', model_name: model.name ?? null },
  ...
  return { snapshotId: snapshot.id, modelCount: models.length, insertedModels, changedPrices, expiredModels };
}
