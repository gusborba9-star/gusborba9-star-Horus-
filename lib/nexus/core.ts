...
export type NexusCoreInput = { intent: string; context: string[]; budgetBrl: number; capability?: string; liveCatalog?: ModelCatalogEntry[]; };
export type NexusCorePlan = {
  optimized: OptimizedPrompt;
  model: RoutedModel;
  context: string[];
  routing: { providerNeutral: true; objective: 'BEST_FIT'; factors: readonly ['quality', 'cost', 'latency', 'reliability', 'context', 'modality', 'task_profile']; };
};
...
export async function resolveNexusPlan(service: SupabaseClient, input: NexusCoreInput): Promise<NexusCorePlan> {
  ...
  return { optimized, model, context, routing: { providerNeutral: true, objective: 'BEST_FIT', factors: ['quality', 'cost', 'latency', 'reliability', 'context', 'modality', 'task_profile'] } };
}
...
export function buildNexusExecutionMetadata(plan: NexusCorePlan) {
  return {
    ...
    execution_contract: plan.model.executionContract,
