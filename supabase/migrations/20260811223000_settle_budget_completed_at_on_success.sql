...
begin
 ...
 if v_release > 0 then update public.execution_budgets set remaining_cost_brl=least(maximum_total_cost_brl,remaining_cost_brl+v_release) where id=v_budget.id; end if;
 insert into public.execution_usage(attempt_id,input_tokens,output_tokens,reasoning_tokens,cached_input_tokens,request_units,image_units,actual_provider_cost_brl,actual_total_cost_brl,raw_usage) values(p_attempt_id,p_input_tokens,p_output_tokens,p_reasoning_tokens,p_cached_input_tokens,p_request_units,p_image_units,p_actual_cost_brl,p_actual_cost_brl,p_raw_usage) on conflict(attempt_id) do update set input_tokens=excluded.input_tokens,output_tokens=excluded.output_tokens,reasoning_tokens=excluded.reasoning_tokens,cached_input_tokens=excluded.cached_input_tokens,request_units=excluded.request_units,image_units=excluded.image_units,actual_provider_cost_brl=excluded.actual_provider_cost_brl,actual_total_cost_brl=excluded.actual_total_cost_brl,raw_usage=excluded.raw_usage,recorded_at=now();
 ...
 if p_status='SUCCEEDED' then v_budget_status:='SETTLED'; elsif p_status in ('FAILED','CANCELED') then select * into v_budget from public.execution_budgets where id=v_attempt.budget_id for update; v_budget_status:=case when v_budget.remaining_attempts<=0 then 'EXHAUSTED' else 'AUTHORIZED' end; else v_budget_status:='RUNNING'; end if;
 update public.execution_budgets set status=v_budget_status, completed_at=case when v_budget_status in ('SETTLED','EXHAUSTED') then coalesce(completed_at,now()) else completed_at end where id=v_attempt.budget_id;
 return v_attempt;
end;
$function$;

update public.execution_budgets b set completed_at=a.completed_at from public.execution_attempts a where a.budget_id=b.id and b.completed_at is null and b.status='SETTLED' and a.status='SUCCEEDED' and a.completed_at is not null;
update public.execution_budgets b set completed_at=a.completed_at from public.execution_attempts a where a.budget_id=b.id and b.completed_at is null and b.status='EXHAUSTED' and a.status in ('FAILED','CANCELED') and a.completed_at is not null;
