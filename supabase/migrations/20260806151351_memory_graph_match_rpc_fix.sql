...
begin
  ...
  with candidates as (
    ...
    update public.memory_graph_nodes m set last_accessed_at=now(),retrieval_count=m.retrieval_count+1 from candidates c where m.id=c.id returning m.id
  )
  return query
  ...
  from candidates c join public.memory_graph_nodes m on m.id=c.id
