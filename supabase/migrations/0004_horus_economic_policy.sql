-- Economic policy data. Values are mutable operational data, not application constants.
...
create policy economic_policy_select_authenticated on public.economic_policy
  for select to authenticated using (true);

-- Writes are privileged system operations only.
