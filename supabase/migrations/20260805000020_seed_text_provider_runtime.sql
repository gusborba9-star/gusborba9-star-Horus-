...
on conflict (provider_id, id) do update set
  capability = excluded.capability, input_price_per_million = excluded.input_price_per_million,
  output_price_per_million = excluded.output_price_per_million, currency = excluded.currency,
  context_window = excluded.context_window, enabled = true, price_verified_at = excluded.price_verified_at,
  canonical_slug = excluded.canonical_slug, request_price = excluded.request_price, image_price = excluded.image_price,
  reasoning_price_per_million = excluded.reasoning_price_per_million, cached_input_price_per_million = excluded.cached_input_price_per_million,
  cache_write_price_per_million = excluded.cache_write_price_per_million, max_completion_tokens = excluded.max_completion_tokens,
  supported_parameters = excluded.supported_parameters, input_modalities = excluded.input_modalities,
  output_modalities = excluded.output_modalities, expiration_date = excluded.expiration_date,
...
where not exists (
  select 1 from public.pricing_snapshots
  where source = 'openrouter'
    and payload_hash = md5('openrouter|google/gemini-2.5-flash-lite|0.10|0.40|0.01|0.03|2026-08-05')
);
