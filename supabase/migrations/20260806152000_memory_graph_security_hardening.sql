...
create or replace function public.memory_graph_touch_updated_at()
returns trigger language plpgsql set search_path = public as $$
begin
  new.updated_at = now();
  if new.content_hash = '' then new.content_hash = md5(new.content); end if;
  return new;
end;
$$;
...
declare query_vector extensions.vector := query_embedding::text::extensions.vector;
begin
  ...
  with candidates as (
    select m.id as candidate_id from public.memory_graph_nodes m
  ...
  update public.memory_graph_nodes m set last_accessed_at=now(),retrieval_count=m.retrieval_count+1 where m.id in(select c.candidate_id from candidates c);
  return query select m.id,m.node_type,m.content,case when m.embedding is null then null else to_jsonb(m.embedding::text) end,m.importance,m.metadata,m.owner_scope,m.user_id,m.organization_id,m.lifecycle_state,1-(m.embedding <=> query_vector),case when coalesce(m.last_accessed_at,m.created_at)>=now()-interval '7 days' or m.retrieval_count>=1 then 'HOT' else 'COLD' end,m.created_at,m.expires_at
  from public.memory_graph_nodes m
  where m.embedding is not null and m.invalidated_at is null and (m.expires_at is null or m.expires_at>now()) and m.lifecycle_state='ACTIVE'
    and (m.owner_scope='SYSTEM' or (m.owner_scope='USER' and requested_user_id is not null and m.user_id=requested_user_id) or (m.owner_scope='ORGANIZATION' and requested_organization_id is not null and m.organization_id=requested_organization_id))
    and 1-(m.embedding <=> query_vector)>=match_threshold
    and (include_cold or coalesce(m.last_accessed_at,m.created_at)>=now()-interval '7 days' or m.retrieval_count>=1)
  order by (1-(m.embedding <=> query_vector)) desc,m.importance desc,m.retrieval_count desc,m.created_at desc limit match_count;
end;
$$;
