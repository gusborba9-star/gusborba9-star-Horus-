    ...
    <header className={`fixed top-0 inset-x-0 z-50 transition-all duration-300 ${scrolled ? 'bg-black/40 backdrop-blur-md border-b border-white/5' : 'bg-transparent border-b border-transparent'}`}>
      <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-8">
          ...
          <Link href="#precos" className="text-sm font-medium text-white/60 hover:text-white transition-colors">
            Preços
          </Link>
          <button className="px-5 py-2 bg-white text-black text-sm font-bold rounded-full hover:bg-cyan-400 transition-colors">
            Agendar Demo
          </button>
        </nav>
      ...
      {mobileMenuOpen && (
        ...
        >
          ...
          <Link href="#precos" className="text-base font-medium text-white/60 py-2" onClick={() => setMobileMenuOpen(false)}>Preços</Link>
          <hr className="border-white/5 my-2" />
          <button className="w-full px-5 py-3 bg-white text-black rounded-xl text-base font-bold">Agendar Demo</button>
        </motion.div>
      )}
    </header>
  );
