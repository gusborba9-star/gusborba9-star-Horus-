...
export default function StudioResultPage({ params }: { params: Promise<{ resultId: string }> }) {
  ...
  async function approve() {
    if (!result?.revision_id || approving) return;
    setApproving(true); setError('');
    try {
      const { data } = await supabase.auth.getSession();
      const token = data.session?.access_token;
      if (!token) throw new Error('AUTHENTICATION_REQUIRED');
      const projectResponse = await fetch(`/api/studio/results/${result.id}`, { headers: { Authorization: `Bearer ${token}` } });
      const projectPayload = await projectResponse.json();
      const projectId = projectPayload.result?.project_id;
      if (!projectId) throw new Error('PROJECT_CONTEXT_MISSING');
      const response = await fetch(`/api/studio/projects/${projectId}/revisions/${result.revision_id}/approval`, { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify({ state: 'APPROVED' }) });
      const payload = await response.json();
      if (!response.ok) throw new Error(payload.error ?? 'APPROVAL_FAILED');
      setApproved(true);
    } catch (error) { setError(error instanceof Error ? error.message : 'APPROVAL_FAILED'); }
  ...
  return <main className="min-h-screen bg-[#080808] text-white p-6 lg:p-10"><div className="max-w-5xl mx-auto space-y-6">
    ...
    <section className="rounded-3xl border border-white/10 bg-white/[0.02] p-6 lg:p-8">
      <div className="flex items-center justify-between mb-6"><div><div className="text-[10px] uppercase tracking-[0.25em] text-white/35">{result.capability}</div><h1 className="text-2xl font-light mt-2">Preview do resultado</h1></div><span className="text-[10px] uppercase tracking-widest text-emerald-300">{approved ? 'APPROVED' : result.status}</span></div>
      ...
      <div className="flex flex-wrap gap-3 mt-6"><button onClick={() => router.push('/dashboard/studio')} className="px-4 py-3 rounded-xl border border-white/10 text-xs uppercase tracking-widest text-white/60">Refinar</button><button onClick={() => router.push('/dashboard/studio')} className="px-4 py-3 rounded-xl border border-white/10 text-xs uppercase tracking-widest text-white/60"><RefreshCw className="w-4 h-4 inline mr-2" />Gerar novamente</button><button disabled={approved || approving} onClick={() => void approve()} className="px-5 py-3 rounded-xl bg-[#D4AF37] text-black font-bold text-xs uppercase tracking-widest disabled:opacity-40">{approved ? 'Aprovado' : approving ? 'Aprovando…' : 'Aprovar'}</button></div>
    </section>
  </div></main>;
}
