...
interface Revision { id: string; version: number; change_class: string; approval_state: string; optimized_spec: Record<string, unknown>; preview?: { status?: string; url?: string | null; verified?: boolean; deploymentId?: string; resultId?: string; resultType?: string }; deployment?: Record<string, unknown>; created_at: string; }
interface StudioResult { id: string; capability: string; result_type: string; status: string; content_text: string | null; artifact_url: string | null; }
...
export default function StudioHome() {
  ...
  const selectProject = useCallback((project: Project | null) => { selectedIdRef.current = project?.id ?? null; setSelected(project); setResult(null); }, []);
  ...
  async function runNexus() {
    ...
    try {
      const response = await authFetch(`/api/studio/projects/${selected.id}/nexus`, { method: 'POST', body: JSON.stringify({ message: request }) });
      const payload = await response.json();
      if (!response.ok) throw new Error(payload.error ?? 'NEXUS_RESULT_FAILED');
      if (payload.result) setResult(payload.result);
      setRequest('');
      setMessage(payload.nexus?.capability ? `Nexus: ${payload.nexus.capability}. Resultado pronto.` : 'Resultado pronto.');
      await loadRevisions(selected.id);
      await loadProjects();
    } catch (error) { setMessage(error instanceof Error ? error.message : 'NEXUS_RESULT_FAILED'); }
  ...
  async function planRevision() {
    if (!selected || !request.trim()) return;
    setBusy(true); setMessage('');
    try { const response = await authFetch(`/api/studio/projects/${selected.id}/revisions`, { method: 'POST', body: JSON.stringify({ prompt: request }) }); const payload = await response.json(); if (!response.ok) throw new Error(payload.error ?? 'REVISION_PLAN_FAILED'); setRequest(''); setMessage(`Revision ${payload.revision.version} planejada: ${payload.revision.change_class}.`); await loadRevisions(selected.id); await loadProjects(); }
  ...
  function lifecycle(revision: Revision) {
    ...
    const previewReady = revision.preview?.status === 'READY' && !revision.preview?.resultId;
    ...
    if (!previewReady && !revision.preview?.resultId) return { label: 'Criar Preview', key: 'preview' as ActionKey, run: () => void execute(revision, 'PREVIEW', 'preview') };
    if (revision.preview?.resultId) return { label: 'Resultado pronto', key: 'preview' as ActionKey, run: () => { if (revision.preview?.url) window.open(revision.preview.url, '_self'); } };
  ...
  return <div className="h-full overflow-y-auto bg-[#080808] text-[#FAFAFA]"><div className="max-w-[1500px] mx-auto p-6 lg:p-10 space-y-8">
    <header className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-5"><div><div className="flex items-center gap-2 text-[#D4AF37] text-[10px] font-bold tracking-[0.3em] uppercase mb-3"><BrainCircuit className="w-4 h-4" /> Nexus Project Execution</div><h1 className="text-4xl md:text-6xl font-light tracking-tight">Studio Hórus™</h1><p className="text-sm text-white/40 max-w-3xl mt-4 leading-relaxed">Você declara a intenção; o Nexus contextualiza, classifica, roteia e produz o resultado quando a capacidade é suportada.</p></div><button onClick={() => void loadProjects()} className="px-4 py-2.5 rounded-xl border border-white/10 text-xs uppercase tracking-widest text-white/60 hover:text-white flex items-center gap-2"><RefreshCw className="w-4 h-4" /> Atualizar</button></header>
    <section className="grid grid-cols-1 xl:grid-cols-[340px_minmax(0,1fr)] gap-6"><aside className="rounded-3xl border border-white/10 bg-white/[0.02] p-5"><div className="flex items-center justify-between mb-4"><span className="text-[10px] font-bold uppercase tracking-[0.25em] text-white/40">Projetos</span><span className="text-[10px] text-[#D4AF37]">{projects.length}</span></div><div className="space-y-2 max-h-[520px] overflow-y-auto">{loading ? <div className="text-xs text-white/30 p-4">Carregando workspace…</div> : projects.map((project) => <button key={project.id} onClick={() => selectProject(project)} className={`w-full text-left p-4 rounded-2xl border ${selected?.id === project.id ? 'border-[#D4AF37]/30 bg-[#D4AF37]/[0.06]' : 'border-white/5 bg-white/[0.015]'}`}><div className="text-sm font-semibold truncate">{project.name}</div><div className="text-[11px] text-white/35 mt-1 line-clamp-2">{project.objective}</div><div className="flex gap-2 mt-3 text-[9px] uppercase tracking-widest text-white/30"><span>{project.status}</span><span>·</span><span>{project.environment}</span></div></button>)}</div><div className="border-t border-white/10 mt-5 pt-5 space-y-3"><input value={name} onChange={(e) => setName(e.target.value)} placeholder="Nome do projeto" className="w-full rounded-xl bg-black/30 border border-white/10 px-3 py-2.5 text-sm outline-none" /><textarea value={objective} onChange={(e) => setObjective(e.target.value)} placeholder="O que você quer construir?" rows={3} className="w-full rounded-xl bg-black/30 border border-white/10 px-3 py-2.5 text-sm outline-none resize-none" /><button disabled={busy || !name.trim() || !objective.trim()} onClick={() => void createProject()} className="w-full rounded-xl bg-[#D4AF37] text-black font-bold text-xs uppercase tracking-widest py-3 disabled:opacity-40"><Plus className="w-4 h-4 inline mr-2" />Novo projeto</button></div></aside>
      ...
      <section className="rounded-3xl border border-white/10 bg-white/[0.02] p-6"><div className="flex items-center gap-2 mb-3"><Sparkles className="w-4 h-4 text-[#D4AF37]" /><span className="text-[10px] font-bold uppercase tracking-[0.25em] text-white/50">Nexus</span></div><textarea value={request} onChange={(e) => setRequest(e.target.value)} placeholder="Ex.: Crie uma imagem de uma mulher andando na chuva." rows={4} className="w-full rounded-2xl bg-black/30 border border-white/10 p-4 text-sm outline-none resize-none" /><div className="flex justify-between gap-3 mt-4"><p className="text-[11px] text-white/30">A conversa preserva o objetivo do projeto e os resultados anteriores.</p><div className="flex gap-2"><button disabled={busy || !request.trim()} onClick={() => void planRevision()} className="px-4 py-3 rounded-xl border border-white/10 text-white/60 text-xs uppercase tracking-widest disabled:opacity-40">Planejar</button><button disabled={busy || !request.trim()} onClick={() => void runNexus()} className="px-5 py-3 rounded-xl bg-[#D4AF37] text-black font-bold text-xs uppercase tracking-widest disabled:opacity-40">Gerar resultado <ArrowRight className="w-4 h-4 inline ml-1" /></button></div></div></section>
      {result ? <section className="rounded-3xl border border-[#D4AF37]/20 bg-[#D4AF37]/[0.03] p-6 lg:p-8"><div className="flex items-center justify-between mb-5"><div><div className="text-[9px] uppercase tracking-[0.3em] text-[#D4AF37]">Resultado Nexus · {result.capability}</div><h3 className="text-2xl font-light mt-2">Preview do resultado</h3></div><CheckCircle2 className="w-5 h-5 text-emerald-300" /></div>{result.result_type === 'IMAGE' && result.artifact_url ? <div className="rounded-2xl overflow-hidden border border-white/10 bg-black"><img src={result.artifact_url} alt="Resultado gerado pelo Nexus" className="w-full max-h-[720px] object-contain" /></div> : result.content_text ? <pre className="whitespace-pre-wrap rounded-2xl bg-black/40 border border-white/10 p-5 text-sm leading-relaxed">{result.content_text}</pre> : <div className="rounded-2xl border border-white/10 p-10 text-sm text-white/40">O provider não retornou um conteúdo visualizável.</div>}<div className="flex flex-wrap gap-2 mt-5"><button onClick={() => { if (selected) void runNexus(); }} className="px-4 py-3 rounded-xl border border-white/10 text-xs uppercase tracking-widest text-white/60">Gerar novamente</button><button onClick={() => setMessage('Refinamento enviado ao Nexus na próxima geração.')} className="px-4 py-3 rounded-xl border border-white/10 text-xs uppercase tracking-widest text-white/60">Refinar</button><button onClick={() => setMessage('Resultado pronto para a etapa de aprovação do lifecycle aplicável.')} className="px-5 py-3 rounded-xl bg-[#D4AF37] text-black font-bold text-xs uppercase tracking-widest">Aprovar</button></div></section> : null}
      <section className="rounded-3xl border border-white/10 bg-white/[0.02] p-6"><div className="flex items-center justify-between mb-5"><span className="text-[10px] font-bold uppercase tracking-[0.25em] text-white/40">Revision Engine</span><span className="text-[10px] text-white/25">PREVIEW → VERIFY → APPROVAL → STAGING → VERIFY → PRODUCTION → VERIFY → DELIVERY → ROLLBACK</span></div><div className="space-y-3">{revisions.length === 0 ? <div className="text-xs text-white/30">Nenhuma revisão planejada.</div> : revisions.map((revision) => { const next = lifecycle(revision); const running = action?.id === revision.id; const deployment = revision.deployment ?? {}; const staging = (deployment.staging ?? {}) as Record<string, unknown>; const production = (deployment.production ?? {}) as Record<string, unknown>; const delivery = (deployment.delivery ?? {}) as Record<string, unknown>; return <div key={revision.id} className="p-4 rounded-2xl border border-white/5 bg-black/20 space-y-4"><div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4"><div><div className="text-sm">Revision {revision.version} <span className="text-white/30">· {revision.change_class}</span></div><div className="text-[10px] text-white/30 mt-1">{revision.approval_state}</div><div className="text-[9px] uppercase tracking-widest text-white/25 mt-2">Preview · {revision.preview?.status ?? 'NOT_CREATED'}{revision.preview?.verified ? ' · VERIFIED' : ''}{revision.preview?.resultId ? ' · RESULT' : ''} · Staging · {String(staging.status ?? 'LOCKED')}{staging.verified ? ' · VERIFIED' : ''} · Production · {String(production.status ?? 'LOCKED')}{production.verified ? ' · VERIFIED' : ''} · Delivery · {String(delivery.status ?? 'NOT_DELIVERED')}</div></div><div className="flex items-center gap-2 flex-wrap">{revision.preview?.status === 'READY' && revision.preview.url ? <a href={revision.preview.url.startsWith('/') ? revision.preview.url : `https://${revision.preview.url.replace(/^https?:\/\//, '')}`} target="_blank" rel="noreferrer" className="px-4 py-2.5 rounded-xl border border-white/10 text-[9px] uppercase tracking-widest text-white/60">Abrir Preview</a> : null}<button disabled={running} onClick={next.run} className="px-4 py-2.5 rounded-xl bg-[#D4AF37] text-black font-bold text-[9px] uppercase tracking-widest disabled:opacity-40">{running ? 'Executando…' : next.label}</button></div></div></div>; })}</div></section></>}</main></section>
