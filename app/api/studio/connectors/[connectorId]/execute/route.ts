...
import { CONNECTOR_PERMISSIONS } from '@/lib/studio/types';

function hasPermission(permissions: unknown, required: string) {
...
export async function POST(request: Request, context: { params: Promise<{ connectorId: string }> }) {
  try {
    ...
    const { data: connector, error } = await client.from('studio_connectors').select('id,provider,permissions,status,secret_ref,metadata').eq('id', connectorId).single();
    if (error || !connector || connector.status !== 'CONNECTED' || !connector.secret_ref) return NextResponse.json({ success: false, error: 'CONNECTOR_NOT_AVAILABLE' }, { status: 404 });

    const operation = typeof body.operation === 'string' ? body.operation : '';
    const token = await getSecret(connector.secret_ref);
    let result: unknown;
    ...
    if (connector.provider === 'github') {
      ...
      result = await upstream(`https://api.github.com/repos/${repo}`, token);
      ...
      result = await upstream(`https://api.vercel.com/v9/projects/${encodeURIComponent(project)}`, token);
      ...
      result = await upstream(`https://api.supabase.com/v1/projects/${encodeURIComponent(projectRef)}`, token);
    } else {
      throw new Error('EXTERNAL_API_REQUIRES_EXPLICIT_ADAPTER');
    }

    await client.from('studio_connectors').update({ last_used_at: new Date().toISOString() }).eq('id', connectorId);
    return NextResponse.json({ success: true, provider: connector.provider, operation, result });
  } catch (error) {
