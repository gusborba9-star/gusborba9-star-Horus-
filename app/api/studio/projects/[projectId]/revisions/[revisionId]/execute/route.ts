  ...
  try {
    ...
    if (existing?.id) {
      ...
      const { data: execution, error: executionError } = await service.from('studio_executions').insert({
        ...
        approval_required: revision.approval_state === 'PENDING',
        approval_granted: revision.approval_state === 'APPROVED',
        estimated_cost_brl: TEST_BUDGET_BRL,
        actual_cost_brl: null,
        request: { environment: 'PREVIEW', revisionId },
        result: {},
        operation_id: crypto.randomUUID(),
        execution_log_id: executionLogId,
        complexity: revision.change_class === 'MAJOR' ? 'HIGH' : 'MEDIUM',
        environment: 'PREVIEW',
        idempotency_key: idempotencyKey,
        risk: revision.change_class === 'MAJOR' ? 'HIGH' : 'LOW',
        optimized_spec: revision.optimized_spec,
    ...
    const { data: connector, error: connectorError } = await client.from('studio_connectors').select('id,provider,permissions,status,secret_ref,metadata,expires_at,revoked_at').eq('project_id', projectId).eq('provider', 'vercel').eq('status', 'CONNECTED').order('created_at', { ascending: false }).limit(1).maybeSingle();
    ...
    const { error: reconciliationError } = await service.rpc('reconcile_horus_execution_attempt', {
      ...
      p_request_units: 1,
      p_image_units: 0,
      p_provider_request_id: deploymentId,
      p_actual_provider: 'vercel',
      p_actual_model: 'deployment',
