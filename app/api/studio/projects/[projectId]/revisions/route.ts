...
export async function POST(request: Request, context: { params: Promise<{ projectId: string }> }) {
  try {
    ...
    const { data: revision, error } = await client.from('studio_project_revisions').insert({
      ...
      diff: { intent: prompt, changeClass: spec.changeClass },
    ...
    const { error: updateError } = await client.from('studio_projects').update({ status: 'PLANNING', capabilities: spec.capabilities, intelligence_snapshot: spec }).eq('id', projectId);
