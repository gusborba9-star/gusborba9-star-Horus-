...
function buildConversationContext(results: any[]) {
  return results.flatMap((result) => [
    typeof result.provider_metadata?.user_prompt === 'string' ? `Previous user intent: ${result.provider_metadata.user_prompt}` : '',
    typeof result.content_text === 'string' ? `Previous result text: ${result.content_text}` : '',
    typeof result.artifact_url === 'string' ? 'A previous artifact was generated.' : '',
...
export async function POST(request: Request, context: { params: Promise<{ projectId: string }> }) {
  try {
    ...
    const { data: previousResults } = await client.from('studio_results')
      .select('id,content_text,artifact_url,provider_metadata,created_at')
