...
export async function POST(request: Request, context: { params: Promise<{ projectId: string }> }) {
  try {
    ...
    return NextResponse.json({ success: true, revision, result: storedResult, preview: resultPreview, nexus: { workType: spec.workType, capability: requestedCapability, providerId: plan.model.providerId, modelId: plan.model.modelId, source: plan.model.source } }, { status: 201 });
  } catch (error) {
