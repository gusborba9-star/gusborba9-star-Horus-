...
export async function POST(req: NextRequest) {
  try {
    const { objetivo, profile } = (await req.json()) as RequestBody;

    if (!objetivo || !profile) {
      return NextResponse.json({ success: false, error: 'Objetivo e Profile são obrigatórios.' }, { status: 400 });
    }

    const megaObjetivo = `
Você é um Membro Cognitivo Polimórfico de Alta Performance operando pelo Hórus OS.
Perfil: ${profile.name}
Nicho: ${profile.niche}
Papel: ${profile.role}
...
`.trim();
      ...
      try {
        const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
          ...
          body: JSON.stringify({
            model: model,
            messages: [
              { role: 'system', content: megaObjetivo },
    ...
    console.warn('[HorusRouter] Todos os módulos OpenRouter falharam. Acionando backup Gemini...');
    try {
      ...
      return NextResponse.json({
        success: false,
        error: 'Todos os módulos falharam.',
